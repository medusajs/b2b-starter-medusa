/**
 * Full extraction script for Edeltec
 * Saves products to JSON and CSV
 * Usage: EMAIL=user@example.com PASSWORD=password npx tsx extract-edeltec-full.ts
 */

import EdeltecMCPServer from './server.js';
...
async function extractFull(): Promise<void> {
  const server = new EdeltecMCPServer({
    name: 'edeltec',
    version: '1.0.0',
  ...
  try {
    console.log('🚀 Starting Edeltec full extraction...');
    ...
    // Save JSON
    const jsonPath = 'edeltec-catalog-full.json';
    await fs.writeFile(jsonPath, JSON.stringify(result, null, 2));
    console.log(`✅ Saved ${result.products.length} products to ${jsonPath}`);
    
    // Save CSV
    const csvPath = 'edeltec-catalog-full.csv';
