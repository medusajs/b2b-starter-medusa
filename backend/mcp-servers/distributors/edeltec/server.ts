/**
 * Edeltec B2B MCP Server
 * URL: https://www.edeltec.com.br
 */
...
export class EdeltecMCPServer extends BaseMCPServer {
  ...
  protected async getProduct(sku: string): Promise<Product | null> {
    ...
    try {
      await this.page.goto(`https://www.edeltec.com.br/produto/${sku}`, { waitUntil: 'networkidle' });
      ...
      this.logger.error({ error, sku }, 'Failed to get Edeltec product');
      return null;
    }
  }
  ...
  async extractProducts(config: ExtractionConfig): Promise<ExtractionResult> {
    try {
      this.logger.info({ config }, 'Starting Edeltec product extraction');
      ...
      await pRetry(
        async () => {
          const session = await this.authenticate(
            process.env.`EMAIL_EDELTEC` || '',
            process.env.`PASSWORD_EDELTEC` || ''
          );
      ...
      this.logger.error({ error }, 'Extraction failed for Edeltec');
      throw error;
    }
  }
...
export default EdeltecMCPServer;
