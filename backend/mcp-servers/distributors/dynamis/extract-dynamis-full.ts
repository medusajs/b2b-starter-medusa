/**
 * Full extraction script for Dynamis
 * Saves products to JSON and CSV
 * Usage: EMAIL=user@example.com PASSWORD=password npx tsx extract-dynamis-full.ts
 */

import DynamisMCPServer from './server.js';
...
async function extractFull(): Promise<void> {
  const server = new DynamisMCPServer({
    name: 'dynamis',
    version: '1.0.0',
  ...
  try {
    console.log('🚀 Starting Dynamis full extraction...');
    ...
    // Save JSON
    const jsonPath = 'dynamis-catalog-full.json';
    await fs.writeFile(jsonPath, JSON.stringify(result, null, 2));
    console.log(`✅ Saved ${result.products.length} products to ${jsonPath}`);
    
    // Save CSV
    const csvPath = 'dynamis-catalog-full.csv';
