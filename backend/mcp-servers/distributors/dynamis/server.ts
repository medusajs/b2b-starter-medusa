/**
 * Dynamis B2B MCP Server
 * URL: https://www.dynamis.com.br
 */
...
export class DynamisMCPServer extends BaseMCPServer {
  ...
  protected async getProduct(sku: string): Promise<Product | null> {
    ...
    try {
      await this.page.goto(`https://www.dynamis.com.br/produto/${sku}`, { waitUntil: 'networkidle' });
      ...
      this.logger.error({ error, sku }, 'Failed to get Dynamis product');
      return null;
    }
  }
  ...
  async extractProducts(config: ExtractionConfig): Promise<ExtractionResult> {
    try {
      this.logger.info({ config }, 'Starting Dynamis product extraction');
      ...
      await pRetry(
        async () => {
          const session = await this.authenticate(
            process.env.`EMAIL_DYNAMIS` || '',
            process.env.`PASSWORD_DYNAMIS` || ''
          );
      ...
      this.logger.error({ error }, 'Extraction failed for Dynamis');
      throw error;
    }
  }
...
export default DynamisMCPServer;
