/**
 * Full extraction script for Solfácil
 * Saves products to JSON and CSV
 * Usage: EMAIL=user@example.com PASSWORD=password npx tsx extract-solfacil-full.ts
 */

import SolfácilMCPServer from './server.js';
...
async function extractFull(): Promise<void> {
  const server = new SolfácilMCPServer({
    name: 'solfacil',
    version: '1.0.0',
  ...
  try {
    console.log('🚀 Starting Solfácil full extraction...');
    ...
    // Save JSON
    const jsonPath = 'solfacil-catalog-full.json';
    await fs.writeFile(jsonPath, JSON.stringify(result, null, 2));
    console.log(`✅ Saved ${result.products.length} products to ${jsonPath}`);
    
    // Save CSV
    const csvPath = 'solfacil-catalog-full.csv';
