...
export class OdexMCPServer extends BaseMCPServer {
  ...
  protected async getProduct(sku: string): Promise<Product | null> {
    ...
    try {
      await this.page.goto(`https://www.odex.com.br/produto/${sku}`, { waitUntil: 'networkidle' });
      ...
      this.logger.error({ error, sku }, 'Failed to get Odex product');
      return null;
    }
  }
  ...
  async extractProducts(config: ExtractionConfig): Promise<ExtractionResult> {
    try {
      this.logger.info({ config }, 'Starting Odex product extraction');
      ...
      await pRetry(
        async () => {
          const session = await this.authenticate(
            process.env.`EMAIL_ODEX` || '',
            process.env.`PASSWORD_ODEX` || ''
          );
      ...
      this.logger.error({ error }, 'Extraction failed for Odex');
      throw error;
    }
  }
