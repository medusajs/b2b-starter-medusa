/**
 * Full extraction script for Odex
 * Saves products to JSON and CSV
 * Usage: EMAIL=user@example.com PASSWORD=password npx tsx extract-odex-full.ts
 */

import OdexMCPServer from './server.js';
...
async function extractFull(): Promise<void> {
  const server = new OdexMCPServer({
    name: 'odex',
    version: '1.0.0',
  ...
  try {
    console.log('🚀 Starting Odex full extraction...');
    ...
    // Save JSON
    const jsonPath = 'odex-catalog-full.json';
    await fs.writeFile(jsonPath, JSON.stringify(result, null, 2));
    console.log(`✅ Saved ${result.products.length} products to ${jsonPath}`);
    
    // Save CSV
    const csvPath = 'odex-catalog-full.csv';
