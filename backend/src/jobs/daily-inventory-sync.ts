...
// Environment variables are already loaded by Medusa
                    ...
                    for (const variant of product.variants) {
                        if (variant.sku) {
                            ...
                            if (inventoryMap.has(variant.sku)) {
                                ...
                                // For now, we'll need to recreate the variant with new prices
                                // since Medusa v2 handles prices at creation time
                                try {
                                    // Prepare price data with customer group rules
                                    const priceData = [
                                        {
                                            amount: Math.round(inventoryData.price * 100), // Convert to cents
                                            currency_code: "cad"
                                        }
                                    ];

                                    // Add wholesale prices with customer group rules
                                    if (inventoryData.wholesale_level1 > 0) {
                                        priceData.push({
                                            amount: Math.round(inventoryData.wholesale_level1 * 100),
                                            currency_code: "cad",
                                            rules: {
                                                "customer.group.id": {
                                                    operator: "eq",
                                                    value: customerGroups.wholesale1
                                                }
                                            }
                                        } as any);
                                    }

                                    if (inventoryData.wholesale_level2 > 0) {
                                        priceData.push({
                                            amount: Math.round(inventoryData.wholesale_level2 * 100),
                                            currency_code: "cad",
                                            rules: {
                                                "customer.group.id": {
                                                    operator: "eq",
                                                    value: customerGroups.wholesale2
                                                }
                                            }
                                        } as any);
                                    }

                                    if (inventoryData.wholesale_level3 > 0) {
                                        priceData.push({
                                            amount: Math.round(inventoryData.wholesale_level3 * 100),
                                            currency_code: "cad",
                                            rules: {
                                                "customer.group.id": {
                                                    operator: "eq",
                                                    value: customerGroups.wholesale3
                                                }
                                            }
                                        } as any);
                                    }

                                    // Update variant with new prices
                                    // Note: In Medusa v2, prices are typically set at creation time
                                    // For updates, we may need to delete and recreate the variant
                                    // or use a different approach depending on the exact API
                                    ...
                                    // The exact method depends on whether we need to:
                                    // 1. Delete and recreate the variant
                                    // 2. Use a specific price update endpoint
                                    // 3. Update through the pricing module directly
