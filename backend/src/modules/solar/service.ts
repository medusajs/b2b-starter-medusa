...
class SolarModuleService extends MedusaService({
  ...
  private async persistCalculation(
    ...
    try {
      ...
      const calculation = await this.create("SolarCalculation", {
      ...
      // Create kit records
      for (let i = 0; i < result.kits_recomendados.length; i++) {
        const kit = result.kits_recomendados[i];
        await this.create("SolarCalculationKit", {
  ...
  private async findExistingCalculation(hash: string): Promise<SolarCalculationResult | null> {
    try {
      const calculation = await this.retrieve("SolarCalculation", hash, {
        where: { calculation_hash: hash },
      });
      
      if (!calculation) return null;

      const kits = await this.list("SolarCalculationKit", {
        where: { solar_calculation_id: calculation.id },
  ...
  async getCalculationHistory(filters?: {
    ...
    return await this.list("SolarCalculation", {
      where,
  ...
  async getCalculationStats(): Promise<{
    ...
    const calculations = await this.list("SolarCalculation", {});
