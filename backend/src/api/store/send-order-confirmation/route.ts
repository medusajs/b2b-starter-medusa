...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    console.log("📧 [STORE API] Sending email for order:", order_id);
    ...
    console.log("📤 [STORE API] Sending order placed email...");
    ...
    if (emailSent) {
      console.log("✅ [STORE API] Order confirmation email sent successfully!");
      res.json({ 
        success: true, 
      ...
      console.log("❌ [STORE API] Order confirmation email failed to send");
      res.status(500).json({ 
        success: false, 
    ...
    console.error("❌ [STORE API] Error sending order email:", error);
    res.status(500).json({ 
      success: false, 
