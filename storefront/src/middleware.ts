...
async function setCacheId(request: NextRequest, response: NextResponse) {
  ...
  // Generate a simple cache ID without crypto.randomUUID() for Edge Runtime compatibility
  const newCacheId = `cache_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
